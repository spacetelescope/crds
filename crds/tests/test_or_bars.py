...
def dt_or_bars_certify_bad_value():
    """
    >>> old_state = test_config.setup(url="https://jwst-serverless-mode.stsci.edu")
    >>> CertifyScript("crds.certify data/jwst_miri_ipc.bad-value.fits")()
    CRDS - INFO -  ########################################
    CRDS - INFO -  Certifying 'data/jwst_miri_ipc.bad-value.fits' (1/1) as 'FITS' relative to context 'jwst_0319.pmap'
