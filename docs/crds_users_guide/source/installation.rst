...
Typical onsite users at STScI share a CRDS file cache which contains all CRDS rules and references.  While this
is the default CRDS cache location,  it could be explicitly configured as follows:
...
Typical onsite users do not need to define a CRDS server and will operate by default from information stored 
in /grp/crds/cache.  The default is essentially the same as this explicit setting:

	% setenv CRDS_SERVER_URL  https://crds-serverless-mode.stsci.edu

By switching to a serverless mode,  a user's CRDS cache can be "frozen" guaranteeing a static system with
respect to reference file assignments.

CRDS cache updates and demand-based operation require a CRDS server which can be explicitly defined as follows for HST:
