...
def _node_to_xml(node, eol, indent, preserve_unicode, 
    ...
    if node.nodeType == xml.dom.Node.ELEMENT_NODE:
        ...
        if node.nodeValue.strip() or not delete_whitespace:
            if preserve_unicode:
...
class Xml(XmlNode):
    ...
    def _indent_string(self, s):
        ...
        with by self._indent
        """
        if not self._eol:
            return s
        lines = []
        for line in s.split(self._eol):
            lines.append(self._indent+line)
        return self._eol.join(lines)
    ...
    def __str__(self):
        ...
        for attr in self.attributes:
            attrs.append('%s="%s"' % (attr, self.attributes[attr]))
        if len(attrs):
            attrstr = " " + " ".join(attrs)
        else:
            attrstr = ""
        if not len(self.elements):
            ...
            for elem in self.elements:
                ...
                elems.append(val)
            elemstr = self._eol.join(elems)
            if self._indent != "":
                elemstr = self._indent_string(elemstr)
            if self._eol != "":
                elemstr = self._eol + elemstr + self._eol
