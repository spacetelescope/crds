...
def reftypes_load_type_spec_spec(default_shared_state):
...
def reftypes_load_type_spec_rmap(default_shared_state):
    SPEC_FILE = os.path.join(os.path.abspath(hst.HERE), "specs", "cos_xwlkfile.rmap")
    spec = reftypes.TypeSpec.from_file(SPEC_FILE)
    default_shared_state.cleanup()

def reftypes_hst_load_raw_specs(default_shared_state):
    SPECS = os.path.join(os.path.abspath(hst.HERE), "specs")
    spec = reftypes.load_raw_specs(SPECS)
    default_shared_state.cleanup()

def reftypes_hst_save_json_specs(default_shared_state, caplog, test_temp_dir):
    SPECS = os.path.join(os.path.abspath(hst.HERE), "specs")
    ...
    with caplog.at_level(logging.DEBUG, logger="CRDS"):
        ...
        out = caplog.text
    expected = f" Saved combined type specs to {f}"
    assert expected in out
    default_shared_state.cleanup()

def reftypes_jwst_load_raw_specs(default_shared_state):
    SPECS = os.path.join(os.path.abspath(jwst.HERE), "specs")
    spec = reftypes.load_raw_specs(SPECS)
    default_shared_state.cleanup()

def reftypes_jwst_save_json_specs(default_shared_state):
    """
    >>> SPECS = os.path.join(os.path.abspath(jwst.HERE), "specs")
    >>> specs = reftypes.load_raw_specs(SPECS)
...
def reftypes_roman_load_raw_specs(default_shared_state):
    """
...
def reftypes_roman_save_json_specs(default_shared_state):
    """
    >>> SPECS = os.path.join(os.path.abspath(roman.HERE), "specs")
    >>> specs = reftypes.load_raw_specs(SPECS)
    >>> f = tempfile.NamedTemporaryFile(delete=False)
...
def reftypes_hst_reference_name_to_tpn_infos(default_shared_state):
    """
...
def reftypes_jwst_reference_name_to_tpn_infos(default_shared_state):    # doctest: +ELLIPSIS
    """
...
def reftypes_roman_reference_name_to_tpn_infos(default_shared_state):
    """
...
def reftypes_hst_get_filekinds(default_shared_state):
    """
    >>> types = reftypes.get_types_object("hst")
...
def reftypes_jwst_get_filekinds(default_shared_state):
    """
    >>> types = reftypes.get_types_object("jwst")
...
def reftypes_roman_get_filekinds(default_shared_state):
    """
    >>> types = reftypes.get_types_object("roman")
...
def reftypes_reference_name_to_tpn_text(default_shared_state):
    """
    >>> types = reftypes.get_types_object("hst")
...
def reftypes_reference_name_to_ld_tpn_text(default_shared_state):
    """
    >>> types = reftypes.get_types_object("hst")
    >>> print(types.reference_name_to_tpn_text("data/s7g1700gl_dead.fits"))
    From TPN: cos_dead.tpn
...
def reftypes_get_row_keys_by_instrument(default_shared_state):
    """
    >>> types = reftypes.get_types_object("hst")
...
class CrdsTestCase:
    def __init__(self, CrdsTestConfig):
        return CrdsTestConfig

class TestsReftypes(CrdsTestCase):

    def setUp(self, *args, **keys):
        super(TestsReftypes, self).setUp(*args, **keys)
        self._old_debug = log.set_exception_trap(False)

    def tearDown(self, *args, **keys):
        super(TestsReftypes, self).tearDown(*args, **keys)
...
def main():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(TestsReftypes)
